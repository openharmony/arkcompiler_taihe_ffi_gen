...
public:
    uintptr_t getApplicationContext() {
        throw std::runtime_error("Function InnerBaseContext::getApplicationContext Not implemented");
    }
    uintptr_t getTestContext() {
        throw std::runtime_error("Function InnerBaseContext::getTestContext Not implemented");
    }
    ...
    void setSupportedProcessCache(bool isSupported) {
        std::cout << "setSupportedProcessCache " << (isSupported ? "True" : "False") << std::endl;
    }
