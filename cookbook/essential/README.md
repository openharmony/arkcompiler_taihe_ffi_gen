...
- Taihe 基于注解机制，将约束严格的 `interface` 投影到具有丰富能力的 ArkTS `interface` 或 `class` 上。
  ...
  - `@ctor`: 生成构造方法（仅当对应接口添加了 `@class` 注解时有效）
...
- `function`: 静态函数
  - 支持入参和返回。
  - **不支持**重载，也就是无法将多个函数实现绑定到同一个名字下。但是，可以使用 `@rename` 在 ArkTS 下将多个函数映射到同一个 ArkTS 函数名下，从而实现重载的效果。
...
@ctor("FancyBook")
function MakeFancyBook(): FancyBook;
```
...
- Taihe 使用 `@rename` 来实现函数重载。被 `@rename("foobar")` 修饰的函数会被投影到 ArkTS 的 `foobar` 函数下，从而实现重载的效果。
...
- Taihe 使用 `@promise` 或 `@async` 注解，将同步函数封装为异步版本。可以与 `@rename` 一起使用。
