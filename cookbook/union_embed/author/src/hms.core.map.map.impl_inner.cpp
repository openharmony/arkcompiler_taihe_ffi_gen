...
public:
    ...
    int32_t setIcons_sync(array_view<::hms::core::map::map_inner::icons_type> icons)
    {
        for (auto const &element : icons) {
            switch (element.get_tag()) {
                case ::hms::core::map::map_inner::icons_type::tag_t::type_string:
                    std::cout << "type_string: " << element.get_type_string_ref() << std::endl;
                    break;
    ...
    void setPositions(array_view<array<::hms::core::map::map_inner::Resource>> positions)
    {
