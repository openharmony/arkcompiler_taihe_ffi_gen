...
void release_Tstring(Tstring_header* handle)
{
    if ((handle->flags & SHARED_STR) == 0) {
        if (atomic_ref_count_decrement(&(((shared_Tstring_header*)handle)->count)) == 0) {
            free(handle);
        }
    }
}
...
shared_Tstring_header* precreate_Tstring_on_heap(uint32_t length)
{
    ...
    shared_Tstring_header* header = (shared_Tstring_header*)malloc(bytes_required);
    if (!header) {
...
// stack
void create_Tstring_on_stack(Tstring_header* header, const char* value, uint32_t length)
{
    assert(value);
    assert(length != 0);
...
void Tstring_init_from_string_view(Tstring* str, const char* value, size_t size)
{
...
void Tstring_assign_from_string_view(Tstring* str, const char* value, size_t size)
{
...
void Tstring_assign_from_initializer(Tstring* str, const char* value, size_t size)
{
    Tstring_init_from_char(str, value, (uint32_t)size);
}
...
{
    return str->handle == NULL;
}

// ABI functions
void* Tstring_get_abi(const Tstring* str)
{
    return (void*)(str->handle);
}
