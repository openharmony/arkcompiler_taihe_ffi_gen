...
- `-C sts:keep-name`：是否保留原函数名（不做 camelCase 转换）
- `-C arkts:module-prefix=com.example`：ArkTS 模块前缀，影响 ANI 类型签名
- `-C arkts:path-prefix=src/arkts`：ArkTS 路径前缀，同样影响 ANI 类型签名
...
#### 方案 2.1 在构造 Analysis 时传入 Option

即 `ArkTsOutDir.get(am, pg, options, ...)`。但这破坏了 Analysis 缓存的语义——Analysis 应该和特定的 Declaration 绑定。将 options 同时作为 Key 的一部分并没有意义，因为在一次编译中不可能出现同一个 Declaration 对应多个不同 options 的情况。另外，这种调用方式也会增加使用复杂度，所有调用 `Analysis.get()` 的地方都需要传入 options。

#### 方案 2.2 Analysis 不缓存，变成纯函数

即 `ArkTsOutDir.compute(pg, options) -> result`，调用方每次传入 options 计算。这放弃了 Analysis 系统的核心价值——缓存和去重。而且 Analysis 之间存在依赖链（`PackageAniInfo` → `PackageGroupAniInfo` → `ArkTsOutDir`），这种方案的传染性会导致所有依赖了选项的 Analysis 都需要在调用时手动传入 options，极大增加使用复杂度。

#### 方案 2.3 Backend 在特定阶段将信息注入回 AnalysisManager 或 IR
...
新增 `taihe.driver.options` 模块，其中包含三个主要组件：
...
其中，`OptionStore` 的 multimap 设计参考了 IR 上 Attribute 的存储方式——同一类型的选项可以出现多次（虽然当前的三个 ANI 选项都是单值的，但这为未来的 `--include-path` 类选项预留了空间）。
...
以 `pretty-print` 后端为例，利用上述 Options 系统，引入了三个命令行选项：

- `-C debug:show-resolved`：是否在输出中显示解析后的类型信息
- `-C debug:show-internal`：是否输出标准库等内部定义
- `-C debug:output-target=<default|stderr|stdout|file:path/to/file.log>`：输出目标，允许默认（即在 `generated/idl` 目录下生成格式化后的文件），输出到 stderr、stdout 或者指定的日志文件

尤其以其中的 `output-target` 选项为例，展示了如何定义一个带参数的选项：
...
class DebugOutputTargetOption(AbstractConfigOption):
    ...
    def try_parse(cls, value: str | None, dm: DiagnosticsManager) -> Self | None:
        if value is None:
            ...
            return None
        if value == "default":
            return cls(target_desc=None)
        if value in ("stderr", "stdout"):
            return cls(target_desc=value)
        if value.startswith("file:"):
            path = Path(value[len("file:") :])
            if not path.is_absolute():
                dm.emit(AdhocError("debug:output-target value must be an absolute path"))
                return None
            return cls(target_desc=path)
        dm.emit(AdhocError(f"invalid value for debug:output-target: {value}"))
        return None
```
...
class PrettyPrintBackendConfig(BackendConfig):
    ...
    target_desc: Literal["stderr", "stdout"] | Path | None = None
    ...
    def from_options(cls, opts: OptionStore, dm: DiagnosticsManager) -> Self | None:
        ...
        if opt is None:
            ...
            return None
        show_resolved = opts.get(DebugShowResolvedOption) is not None
        show_internal = opts.get(DebugShowInternalOption) is not None
        return cls(
    ...
    def build(self, ci: CompilerInstance) -> Backend:
        if self.target_desc is None:
            ...
            om = DebugOutputManager(self.target_desc)
        return PrettyPrintBackend(
            output_manager=om,
...
### `Analysis.provide()` 的设计

`provide()` 方法是这套方案的关键拼图。它的作用是在 Analysis 的缓存中预置一个由外部构造的实例，使得后续通过标准的 `Analysis.get()` API 访问时，直接命中这个预置值，而不走 `_create()` 工厂方法。
...
**时机**：`resolve()` 完成（语义 IR 已构建，类型已解析，属性已转换）之后、语义验证之前。

**用途**：在已解析的语义 IR 上添加后端特有的元数据（属性）。典型场景：将命令行选项物化为 per-declaration 的属性，或通过 `Analysis.provide()` 注入配置型分析结果。

**约束（及理由）**：

1. **必须幂等。** 从语义上讲，`post_process()` 的作用应该被限定在「根据后端配置为 IR 自动添加默认属性」的场景，而如果这些属性已经在 IR 中被显式添加，那么就不应再重复添加。因此，如果 IR 已经是处理后的状态，使用相同的后端配置重复处理就不应该再对该 IR 产生任何变化。

   另外，如果将 `post_process()` 修改后的 IR 保存并重新编译，`post_process()` 将再次执行，若非幂等就会产生与编译次数相关的非确定性结果。实际实现中通过 `if StsKeepNameAttr.get(p) is None` 的前置检查保证幂等。

2. **不得修改 IR 结构本身（声明、类型等），只能添加后端特有属性。** 这是为了保证后端隔离性：开启后端 A 不应改变后端 B 看到的 IR 形态。如果 `post_process` 允许修改声明结构（比如添加/删除函数参数），那么后端 B 的代码生成可能因为后端 A 的存在而产生不同结果——这违反了「后端是代码生成的平行输出通道」的基本假设。
...
2. **开启后端不得导致原本合法的代码报错。（待定）** 对于不添加后端特有注解的代码，`validate()` 应当静默通过，这样保证用户可以简单地将已有 IDL 迁移到新后端，而不需要一次性添加大量注解。
...
```
register → inject → parse → resolve → post_process → validate → generate
                                            ↓
                                      (仅添加属性/
                                       注入 Analysis)
                                                          ↓
...
1. 是否需要在 `ConfigOption` 中添加 `DESCRIPTION` 或 `HELP` 字段，以便在诊断中提供更友好的错误提示？（目前解析失败时由工厂函数向诊断器发出自定义的错误消息，但无法直接在命令行直接查看选项的帮助信息）

2. 关于可积累的选项，在目前的设计中，使用时需要写成 `-Copt-name=value1 -Copt-name=value2` 的形式（通过 `OptionStore` 上的 `get_all()` 方法获取）或者 `-Copt-name=value1,value2` 的形式（由选项的 `parse()` 方法自行解析）。是否需要提供类似 `argparse` 中的更高级 API 来配置命令行参数的解析格式？例如允许自定义选项参数的个数和格式，从而支持解析 `-Copt-name value1 value2`, `-Copt-name value1 -Copt-name value2` 等形式。

3. 在当前，有参/无参，单次/多次选项都使用统一的 `AbstractConfigOption` 作为基类，有参和无参通过 `parse()` 传递的 `str` 或 `None` 来区分，`OptionStore` 上则对所有选项同时提供 `get()` 和 `get_all()` 两种访问方式。是否有必要提供一些带有默认方法的子类来区分不同类型的选项，以便提升易用性？（如引入 `FlagOption` 提供 `is_enabled()` 方法，`SingleValueOption` 提供 `get_value()` 方法，`MultiValueOption` 提供 `get_values()` 方法等）
