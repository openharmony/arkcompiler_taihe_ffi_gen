...
ArkTS 1.1 和传统 JavaScript 采用**单线程事件循环模型**，所有对上层函数的调用都必须在同一个主线程中进行。当 Native 代码运行在其它线程时，它不能直接调用 ArkTS 函数，而必须通过事件机制（如 NAPI 的 `napi_send_event`）将调用请求发送到主线程队列中等待执行。
