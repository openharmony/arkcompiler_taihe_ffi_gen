...
其中 `qiid_fptr`（Query Interface ID）接受一个**接口 ID** 作为参数，返回该接口对应的虚表指针；如果对象未实现该接口，则返回 `NULL`。该函数在 C++ 层中由每个具体实现类型特化生成（参见[第 3 节](#3-c-层设计和实现)），其内部通过逐一调用各接口的 `qiid_impl` 函数来查询接口 ID 对应的虚表指针。
...
```c
InterfaceId IBase_iid = ...;
InterfaceId IColor_iid = ...;
InterfaceId IShape_iid = ...;
InterfaceId IFancyObj_iid = ...;
```
...
1. **泛型引擎 (`runtime/object.hpp`)**: 提供了与具体接口无关的、通用的对象创建、生命周期管理和类型转换的模板工具。

2. **接口包装器 (`*.proj.{0,1,2}.hpp`)**: 这些是 IDL 工具根据用户定义的 .idl 文件自动生成的文件。它们为每个 IDL 接口（如 `IBase`, `IFancyObj`）提供了具体的 C++ 包装类如 `IBase`（持有者）, `weak::IBase`（借用者）等，并包含了用于构建 VTable 和 RTTI 的关键 `constexpr` 模板元数据。
...
`object.hpp` 中的泛型引擎定义了若干对 view/holder 句柄类型，它们在**类型擦除程度**和**可访问能力**上各有不同。每对句柄类型中，`view` 类型是借用语义（不管理引用计数），`holder` 类型是持有语义（RAII 管理引用计数），`holder` 继承自对应的 `view`。

下表汇总了所有句柄类型，从类型擦除程度最高到最低排列：

| 句柄类型 | 模板参数 | 成员 | 编译期已知信息 | 运行时可访问能力 |
|---------|---------|------|--------------|----------------|
| `data_{view,holder}` | 无 | `data_ptr` | 无 | 无直接方法访问能力；仅可通过 RTTI 执行哈希、比较和动态转换 |
| `type_{view,holder}<Impl>` | `Impl` | `data_ptr` | 具体实现类类型 | 可通过 `operator->`、`operator*` 直接访问 `Impl` 的所有成员方法（静态分发）；不知道实现了哪些接口 |
| `weak::IXxx` / `IXxx` | 无（由代码生成确定） | `vtbl_ptr` + `data_ptr` | 接口类型 | 可通过 `operator->` 访问该接口定义的方法（虚表分发）；不知道底层的具体实现类类型 |
| `impl_{view,holder}<Impl, Is...>` | `Impl`, `InterfaceTypes...` | `data_ptr` | 具体实现类类型 + 接口列表 | 可通过 `operator->` 直接访问 `Impl` 的所有成员方法（静态分发）；可**隐式**转换为已声明的任何接口或其基接口 |

各类型的详细说明如下：

**`data_{view,holder}`**

完全类型擦除的句柄。仅持有 `data_ptr`，不包含任何编译期类型信息，也没有 `vtbl_ptr`。它是所有 Taihe 对象的最小公共表示，是**动态转换的枢纽**——任何 Taihe 接口对象或实现类容器都可以隐式转换为 `data_{view,holder}`，而从 `data_{view,holder}` 到某个具体接口则需要通过该接口的**显式构造函数**完成动态转换。

**`type_{view,holder}<Impl>`**

保留了具体实现类类型 `Impl` 的句柄。仅持有 `data_ptr`，编译期知道底层对象是 `Impl` 类型，因此可以通过 `operator->` 和 `operator*` 直接访问 `Impl` 的所有方法（静态分发，无虚表查找）。但它不知道对象实现了哪些 Taihe 接口，因此**不能**隐式转换为任何接口类型。它适合于仅需要操作具体实现类的公开方法、而不关心接口层面交互的场景。

**`weak::IXxx` / `IXxx`**（由代码生成产生）

特定 Taihe 接口的句柄，同时持有 `vtbl_ptr` 和 `data_ptr`。编译期知道接口类型但不知道底层的具体实现类类型，因此方法调用通过虚表分发。它可以隐式转换为其父接口类型（静态转换：指针偏移），或者隐式转换为 `data_{view,holder}`（丢弃 `vtbl_ptr`）。从 `data_{view,holder}` 到接口类型的转换是显式的（动态转换：RTTI 查询）。

**`impl_{view,holder}<Impl, InterfaceTypes...>`**

信息量最完整的句柄。仅持有 `data_ptr`，但编译期同时知道具体实现类 `Impl` 和它所实现的接口列表 `InterfaceTypes...`。因此它既可以像 `type_{view,holder}` 一样直接访问 `Impl` 的所有方法（静态分发），也可以隐式转换为声明的任何接口类型或其基接口（编译期计算虚表指针），还可以向下隐式转换为 `type_{view,holder}<Impl>` 以及 `data_{view,holder}`。它是 `taihe::make_holder` 的返回类型。

#### 3.2.2 隐式（静态）转换关系

各句柄类型之间存在明确的隐式转换（向上）关系。所有隐式转换都是静态的，在编译期即可确定，不涉及运行时 RTTI 查询。下图展示了转换方向（箭头表示“可隐式转换为”）：

```
impl_{view,holder}<Impl, Is...>
    ├──→ weak::IXxx / IXxx         (编译期计算 vtbl_ptr，任何 Is... 及其基接口)
    ├──→ type_{view,holder}<Impl>  (丢弃接口信息)
    └──→ data_{view,holder}        (完全类型擦除)

type_{view,holder}<Impl>
    └──→ data_{view,holder}        (丢弃 Impl 类型信息)

weak::IXxx / IXxx
    ├──→ weak::IParent / IParent   (vtbl_ptr 指针偏移)
    └──→ data_{view,holder}        (丢弃 vtbl_ptr)
```

#### 3.2.3 显式（动态）转换关系

- **接口间的动态转换**：从 `{data/type/impl}_{view/holder}` 到 `weak::IXxx` / `IXxx` 的转换是**显式**的，通过接口的 `explicit` 构造函数触发运行时 RTTI 查询（`qiid_fptr`）。如果对象未实现目标接口则 `vtbl_ptr` 为 `NULL`，可通过 `is_error()` 检查。
- **恢复具体类型**：从已经擦除了具体 `Impl` 类型信息的句柄到 `type_{view,holder}<Impl>` 或 `impl_{view,holder}<Impl, ...>` 的转换在当前设计中无法实现。这是因为 `data_ptr` 中不携带编译期的 `Impl` 类型信息，无法安全地恢复。
...
6. `free_data_ptr<Impl>`, `hash_data_ptr<Impl>` 和 `same_data_ptr<Impl>` 是几个非成员模板函数，用于实现对象的销毁、哈希和比较，这里以 `free_data_ptr<Impl>` 为例，它基本可以看作 `make_data_ptr<Impl>` 的逆操作：
    ...
    动态转换表的查找逻辑被封装在 `qiid` 静态函数中，该函数通过折叠表达式依次调用所有接口的 `qiid_impl` 函数，匹配传入的接口 ID 并返回对应的虚表指针。
...
7. 其中的 `qiid_impl<Impl>` 是每个接口类（如 `IColor`, `IShape`）中定义的一个编译期静态模板成员函数，定义在 `*.proj.1.hpp` 中。它接受一个接口 ID 作为参数，通过 if 链逐一比较该接口及其所有父接口的 ID，匹配时返回对应的虚表指针，否则返回 `nullptr`。例如 `IColor` 的 `qiid_impl` 定义如下：

    ```c++
    static constexpr void const *qiid_impl(InterfaceId id) {
      if (id == &object_IColor_i) {
        return &vtbl_impl<Impl>.ftbl_ptr_0;
      }
      if (id == &object_IBase_i) {
...
9. `ftbl_impl<Impl>` 也是接口类的一个编译期静态模板成员变量，它是 `Impl` 类对于该接口的函数表结构体实现。其中的每个函数指针都通过 `invoke.hpp` 中提供的 `taihe::method_calling_convention` 模板生成。该模板接受 `Impl` 类型、`Impl` 的成员方法指针、返回值类型、接口类型和参数类型列表作为模板参数，自动生成一个与 ABI 函数签名匹配的静态方法 `abi_func`，该方法内部将传入的 `self` 对象中的 `data_ptr` 转换回具体的 `Impl*`，将 ABI 类型参数转换为 C++ 类型，调用 `Impl` 上对应的方法，并将返回值从 C++ 类型转换回 ABI 类型：
    ...
    constexpr object_IColor_ftable object::weak::IColor::ftbl_impl = {
      .version = 0,
      .methods = {
        .getColor = &::taihe::method_calling_convention<Impl, &Impl::getColor, ::taihe::string, ::object::weak::IColor>::abi_func,
        .setColor = &::taihe::method_calling_convention<Impl, &Impl::setColor, void, ::object::weak::IColor, ::taihe::string_view>::abi_func,
      },
    };
    ```

10. 上面的 `method_calling_convention` 模板内部使用了 `cast_data_ptr<Impl>` 函数（定义在 `object.hpp` 中）将 `DataBlockHead*` 转换为具体的 `Impl*`，从而可以调用 `Impl` 类中的方法：
...
所有 Taihe 接口对象都可以被隐式转换为 `taihe::data_{view,holder}`，它们定义在 `object.hpp` 中，表示一个任意/未知类型的 Taihe 对象，只包含 `data_ptr`，没有 `vtbl_ptr`。接口类内的转换函数定义在 `IXxx`, `weak::IXxx` 等类内：
...
2. `weak::IShape::virtual_type` 的定义在 `proj.2.hpp` 中，其中包含了 `getShape` 方法的实现。在调用时，它通过 `invoke.hpp` 中提供的 `taihe::call_abi_func` 模板函数来完成 ABI 函数的调用。`call_abi_func` 接受返回值类型和参数类型列表作为模板参数，内部自动完成 C++ 类型到 ABI 类型的参数转换（`into_abi`）、ABI 函数调用、以及 ABI 返回值到 C++ 类型的转换（`from_abi`）：

    ```c++
    struct ::object::weak::IShape::virtual_type {
      ::taihe::string getShape() const & {
        return ::taihe::call_abi_func<::taihe::string, ::object::weak::IShape>(&object_IShape_getShape_m, *reinterpret_cast<::object::weak::IShape const *>(this));
      }

      void setShape(::taihe::string_view s) const & {
        return ::taihe::call_abi_func<void, ::object::weak::IShape, ::taihe::string_view>(&object_IShape_setShape_m, *reinterpret_cast<::object::weak::IShape const *>(this), ::std::forward<::taihe::string_view>(s));
      }
    };
    ```
