...
核心概念有 4 个，`Argument`、`UncheckedAttribute`、`CheckedAttribute`、`AttributeRegistry`
```
AnyAttribute
├── UncheckedAttribute
...
`CompilerInstance` 有一个成员变量为 `AttributeRegistry`
...
`AstConverter` 有一个成员变量为 `AttributeRegistry`
...
2. 在语言后端的 `BackendConfig` 创建对应语言后端的 Backend 阶段，使用 `AttributeRegistry` 的 `register()` 方法将对应语言后端的 Attribute 注册到 `AttributeRegistry`;
