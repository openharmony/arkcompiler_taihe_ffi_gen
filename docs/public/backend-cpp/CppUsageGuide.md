...
>
> 动态转换则需要在运行时检查实际对象是否实现了目标接口。每个 Taihe 接口在二进制中都对应一个接口 ID（IID），当通过 `taihe::make_holder` 创建对象时，对象数据内存的前面会被插入一个指向运行时类型信息（RTTI）的指针，而在 RTTI 中，则包含一个接口 ID 查询函数指针（`qiid_fptr`），该函数接受目标接口的 IID 作为参数，查找该对象实现的所有接口所对应 IID 到相应虚表的映射关系，并返回对应的虚表指针。
...
可以通过 `emplace` 和 `reset` 方法修改或重置可选类型的值。

```cpp
opt1.emplace(1234);  // 将 opt1 的值设置为 1234
opt3.reset();  // 将 opt3 置空
```
