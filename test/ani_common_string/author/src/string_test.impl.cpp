...
constexpr std::size_t THREE_CODE_UNITS = 3;
...
void TestU16StrTestInternal()
{
    ...
    RequireUtf16Equal(std::u16string_view(encodedWithText.substr(ONE_CODE_UNIT, encodedSurrogatePairs.size())),
                      std::u16string_view(surrogatePairs), "UTF-16 concatenated surrogate string substr() mismatch");
}

ExpectedUtf16String TaiheU16StrTest()
{
    try {
        TestU16StrTestInternal();
...
void CommonStringTestU16()
{
    ...
    RequireUtf16Equal(
        std::u16string_view(utf16Text.substr(SAMPLE_SUBSTR_OFFSET, TWO_CODE_UNITS)).substr(FIRST_INDEX, TWO_CODE_UNITS),
...
void CommonStringTestConversion()
{
    ...
    taihe::string_view compareUtf8("a\xF0\x9F\x98\x80", FIVE_CODE_UNITS);
    taihe::u16string_view compareUtf16(u"a\U0001F600", THREE_CODE_UNITS);
    Require(compareUtf8 == compareUtf16, "UTF-8 and UTF-16 strings with equal code points should compare equal");
    Require(::taihe::string_view("a", ONE_CODE_UNIT) < ::taihe::u16string_view(u"ab", TWO_CODE_UNITS),
            "a code-point prefix should compare less than the longer string");
    Require(::taihe::u16string_view(u"b", ONE_CODE_UNIT) > ::taihe::string_view("ac", ONE_CODE_UNIT),
            "comparison should use code-point order across encodings");
    Require(malformedUtf8Common == malformedUtf16Common,
            "malformed strings with equal replacement code points should compare equal");
...
bool EqualProjectedStrings(::taihe::string_view utf8, ::taihe::u16string_view utf16)
{
    return utf8 == utf16 && std::hash<::taihe::string>()(::taihe::string(utf8)) ==
                                std::hash<::taihe::u16string>()(::taihe::u16string(utf16));
}

ExpectedUtf16String Utf8ToUtf16(::taihe::string_view input)
{
    return ::taihe::u16string(::taihe::common_string_view(input));
}

ExpectedUtf8String Utf16ToUtf8(::taihe::u16string_view input)
{
    return ::taihe::string(::taihe::common_string_view(input));
}
...
TH_EXPORT_CPP_API_EqualProjectedStrings(EqualProjectedStrings);
TH_EXPORT_CPP_API_Utf8ToUtf16(Utf8ToUtf16);
TH_EXPORT_CPP_API_Utf16ToUtf8(Utf16ToUtf8);
// NOLINTEND
