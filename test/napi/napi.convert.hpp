...
template<>
struct napi_convert<taihe::core::string> {
    static inline napi_value create(napi_env env, taihe::core::string&& cpp_obj) {
        napi_value js_obj = nullptr;
        napi_create_string_utf8(env, cpp_obj.c_str(), cpp_obj.size(), &js_obj);
        return js_obj;
    }
    
    static inline taihe::core::string get(napi_env env, napi_value js_obj) {
        size_t len_cpp_obj = 0;
        napi_get_value_string_utf8(env, js_obj, nullptr, 0, &len_cpp_obj);
        char char_cpp_obj[len_cpp_obj + 1];
        napi_get_value_string_utf8(env, js_obj, char_cpp_obj, len_cpp_obj + 1, &len_cpp_obj);
        taihe::core::string cpp_obj(char_cpp_obj);
        return cpp_obj;
    }
};

template<typename T>
struct napi_convert<taihe::core::vector<T>> {
    static napi_value create(napi_env env, taihe::core::vector<T>&& cpp_obj) {
        napi_value js_obj = nullptr;
    ...
    static taihe::core::vector<T> get(napi_env env, napi_value js_obj) {
        uint32_t length;
        napi_get_array_length(env, js_obj, &length);
        ::taihe::core::vector<T> cpp_obj;
...
struct napi_convert<taihe::core::map<K, V>> {
    static napi_value create(napi_env env, taihe::core::map_view<K, V>&& cpp_obj) {
        // 获取全局对象
    ...
    static taihe::core::map<K, V> get(napi_env env, napi_value js_obj) {
        taihe::core::map<K, V> cpp_obj;
        ...
        napi_call_function(env, global, array_from_fn, 1, &entries_iter, &entries_array);
...
template<typename T>
struct napi_convert<taihe::core::set<T>> {
    static napi_value create(napi_env env, taihe::core::set<T>&& cpp_obj) {
        // 获取全局对象
        napi_value global = nullptr;
        napi_get_global(env, &global);
    ...
    static taihe::core::set<T> get(napi_env env, napi_value js_obj) {
        taihe::core::set<T> cpp_obj;
        ...
        napi_call_function(env, global, array_from_fn, 1, &values_iter, &values_array);
