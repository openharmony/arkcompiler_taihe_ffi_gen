...
class CodeGenerator(TaiheVisitor):
    ...
    def visit_BasicType(self, node: TaiheAST.BasicType):
        if node.name.text == 'i8':   return 'int8_t'
        if node.name.text == 'i16':  return 'int16_t'
        if node.name.text == 'i32':  return 'int32_t'
        if node.name.text == 'i64':  return 'int64_t'
        if node.name.text == 'u8':   return 'uint8_t'
        if node.name.text == 'u16':  return 'uint16_t'
        if node.name.text == 'u32':  return 'uint32_t'
        if node.name.text == 'u64':  return 'uint64_t'
        if node.name.text == 'f32':  return 'float'
        if node.name.text == 'f64':  return 'double'
        if node.name.text == 'bool': return 'bool'
