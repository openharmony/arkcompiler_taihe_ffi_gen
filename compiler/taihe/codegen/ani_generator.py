...
class EnumTypeANIInfo(AbstractAnalysis[EnumType], AbstractTypeANIInfo):
    ...
    def into_prx_split(
        ...
        target.write(
            f"{' ' * offset}let {prx_tag}: {self.prx_split_type[0]} = {sts_value}.tag;\n"
            f"{' ' * offset}let {prx_value}: {self.prx_split_type[1]} = {sts_value}.value;\n"
        )
...
class ScalarTypeANIInfo(AbstractAnalysis[ScalarType], AbstractTypeANIInfo):
    ...
    def into_ani_split(
        ...
        target.write(
            f"{' ' * offset}{self.ani_split_type} {ani_result} = {cpp_value};\n"
        )
...
class OptionalTypeANIInfo(AbstractAnalysis[OptionalType], AbstractTypeANIInfo):
    ...
    def from_ani_split(
        ...
        item_ty_ani_info = TypeANIInfo.get(self.am, self.t.item_ty)
        item_ty_ani_info.from_ani_boxed(target, offset + 4, env, ani_value, cpp_temp)
        target.write(
    ...
    def from_prx_whole(
        ...
        if val_ty_ani_info.prx_whole_type == val_ty_ani_info.sts_type:
            target.write(
                f"{' ' * offset}let {sts_result}: {val_ty_ani_info.prx_whole_type}[] = {prx_value};\n"
            )
        else:
            target.write(
                f"{' ' * offset}let {sts_result}: {val_ty_ani_info.prx_whole_type}[] = new {val_ty_ani_info.prx_whole_type}[{prx_value}.length];\n"
                f"{' ' * offset}for (let i = 0; i < {prx_value}.length; i++) {{\n"
            )
            val_ty_ani_info.from_prx_whole(
                target, offset + 4, f"{prx_value}[i]", "item"
