...
class STSCodeGenerator:
    ...
    def gen_bigint_util_func(
        ...
        target.writeln(
            f"function __getByteLengthOfBigInt(n: BigInt): int {{",
            f"    if (n == 0n) return 1;",
            f"    let length = 0;",
            f"    let value = n;",
            f"    while (value > 0n) {{",
        ...
        target.writeln(
            f"function __bigUint64ArrayToBigInt(array: BigUint64Array): BigInt {{",
            f"    let result: BigInt = 0n;",
            f"    let arrBuf: ArrayBuffer = new ArrayBuffer(array.length * 8);",
            f"    for (let index = 0; index < array.length; ++index) {{",
            f"        let newindex: int = index * BigUint64Array.BYTES_PER_ELEMENT as int + array.byteOffset as int;",
            f"        if (array.buffer instanceof ArrayBuffer) {{",
            f"            for (let i = 0; i < BigUint64Array.BYTES_PER_ELEMENT as int; ++i) {{",
            f"                let el = (array.buffer as ArrayBuffer).at(newindex + i) as byte;",
            f"                arrBuf.set(i + index * 8, el);",
            f"            }}",
            f"        }} else if (array.buffer instanceof SharedArrayBuffer) {{",
            f"            for (let i = 0; i < BigUint64Array.BYTES_PER_ELEMENT as int; ++i) {{",
            f"                let el = (array.buffer as SharedArrayBuffer).at(newindex + i) as byte;",
            f"                arrBuf.set(i + index * 8, el);",
            f"            }}",
            f"        }} else {{",
            f'            throw new Error("unexpected type of ArrayBufferLike");',
            f"        }}",
            f"    }}",
            f"    for (let index = 0; index < array.length * 8; ++index) {{",
            f"        let num = arrBuf.at(index) as long;",
            f"        num &= 0xff;",
            f"        result |= BigInt(num) << BigInt(8 * index);",
            f"    }}",
            f"    return result;",
            f"}}",
        )
        target.writeln(
            f"function __bigIntToBigUint64Array(value: BigInt): BigUint64Array {{",
            f"    if (value == 0n) {{",
            f"        return new BigUint64Array(new ArrayBuffer(8));",
            f"    }}",
            f"    let byteLen = __getByteLengthOfBigInt(value);",
            f"    let byteArrSize = (byteLen + 7) & ~7;",
            f"    let arrBuf: ArrayBuffer = new ArrayBuffer(byteArrSize);",
            f"    let temp = value;",
            f"    for (let i = 0; i < byteLen; i++) {{",
            f"        let byteNum = (temp & 255n).getLong() as byte;",
            f"        arrBuf.set(i, byteNum);",
            f"        temp = temp >> 8n;",
            f"    }}",
            f"    return new BigUint64Array(arrBuf);",
            f"}}",
        )
