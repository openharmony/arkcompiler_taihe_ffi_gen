...
class AbiHeadersGenerator:
    ...
    def gen_struct_defn_file(
        ...
        with CHeaderWriter(
            ...
            for field in struct.fields:
                ...
                target.add_include(*type_abi_info.defn_headers)
            self.gen_struct_defn(struct, struct_abi_info, target)
    ...
    def gen_union_defn_file(
        ...
        with CHeaderWriter(
            ...
            self.gen_union_defn(union, union_abi_info, target)
    ...
    def gen_iface_defn_file(
        ...
        with CHeaderWriter(
            ...
            self.gen_iface_vtable(iface, iface_abi_info, target)
            self.gen_iface_defn(iface, iface_abi_info, target)
            self.gen_iface_static_cast(iface, iface_abi_info, target)
            self.gen_iface_dynamic_cast(iface, iface_abi_info, target)
        ...
        with CHeaderWriter(
            ...
            for method in iface.methods:
                ...
                if return_ty_ref := method.return_ty_ref:
                    type_abi_info = TypeAbiInfo.get(self.am, return_ty_ref.resolved_ty)
                    target.add_include(*type_abi_info.defn_headers)
            self.gen_iface_ftable(iface, iface_abi_info, target)
            self.gen_iface_methods(iface, iface_abi_info, target)
            for ancestor, info in iface_abi_info.ancestor_dict.items():
                if ancestor is iface:
                    continue
                ancestor_abi_info = IfaceAbiInfo.get(self.am, ancestor)
    ...
    def gen_package_file(self, pkg: PackageDecl):
        ...
        with CSourceWriter(
            ...
            for iface in pkg.interfaces:
                iface_abi_info = IfaceAbiInfo.get(self.am, iface)
                target.add_include(iface_abi_info.defn_header)
                self.gen_iface_iid(iface, iface_abi_info, target)
